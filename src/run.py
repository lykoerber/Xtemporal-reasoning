...
from prompting import read_data, create_prompt, parse_row
...
def run_dataset(dir: str, model, tokenizer, shots: int = 5, output_file: str
                = '', num_instances: int = 5, by_category: bool = True):
    """
    Generate outputs for a given dataset directory.

    Parameters:
        dir (str): The directory containing the dataset files.
        model: timellama model.
        tokenizer: timellama model tokenizer.
        ...
        num_instances (int, optional): The number of instances to sample from
            the dataset or category. Default is 5.
        by_category (bool, optional): Whether or not instances should be
            sampled by category. Default is True.
    ...
    output_file is provided. Each output includes generated outputs,
    ...
    for category, sub_df in sub_dataframes.items():
        ...
        for d in ds_sample.iterrows():
            # file paths for few-shot directories
            if mcq:
                shot_dir = dir.replace('_mcq', '_shots_mcq')
            else:
                shot_dir = dir.replace('_saq', '_shots_saq')
            prompt = create_prompt(d, shots=shots, shot_dir=shot_dir, mcq=mcq)
            try:
